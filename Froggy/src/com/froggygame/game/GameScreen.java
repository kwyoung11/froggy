...
public class GameScreen implements Screen, GestureListener {
	...
	public GameScreen() {
		stage = new Stage();
		froggyGame = new FroggyGame();
		stage.addActor(froggyGame);
	}
	...
	public boolean tap(float x, float y, int count, int button) {
		froggyGame.playerFrog.tryJump();
		return false;
	}

	@Override
	public boolean fling(float velocityX, float velocityY, int button) {
		// if (velocityY < -100) froggyGame.playerCar.tryMoveUp();
		// if (velocityY > 100) froggyGame.playerCar.tryMoveDown();
		return false;
	}
